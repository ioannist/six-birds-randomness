...
python experiments/hashing_toy/run_hashing_toy.py \
  --n_bits 8,12,16,20 \
  --q_list 1,4,16,64,256,1024 \
  --trials 300 \
  --randtest_samples 3000 \
  --randtest_rep_n_bits 16 \
  --seed 20260306
hash_run="$(detect_new_run_dir "$hash_base" "$hash_before")"
rm -f "$hash_before"
echo "hashing_toy run_dir=$hash_run"

# 4) Representation packaging dataset + clustering
...
printf '\nREPRODUCE_ALL_SUMMARY\n'
echo "markov_bench=$markov_run"
echo "budget_curves=$budget_run"
echo "hashing_toy=$hash_run"
echo "rep_packaging_dataset=$rep_run"
echo "rep_packaging_clustering=$cluster_run"
